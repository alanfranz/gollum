...
$(document).ready(function() {
  ...
  if ($('#minibutton-rename-page').length) {
    ...
    if ($('#minibutton-publish-draft').length) {
        $('#minibutton-publish-draft').parent().removeClass('jaws');
        $('#minibutton-publish-draft').click(function(e) {
            e.preventDefault();

            var path = pagePath();
            var oldName = pageName();
            var context_blurb = "Are you sure to publish?."
            ...
            $.GollumDialog.init({
                ...
                OK: function( res ) {
                    ...
                    var name_parts = abspath('_posts', newName);
                    var newPath = name_parts[0];
